...
func (x *TableAzurePostgresqlServersGenerator) GetDataSource() *schema.DataSource {
	return &schema.DataSource{
		Pull: func(ctx context.Context, clientMeta *schema.ClientMeta, client any, task *schema.DataSourcePullTask, resultChannel chan<- any) *schema.Diagnostics {
			svc := client.(*azure_client.Client).AzureServices().PostgreSQL.Servers
...
func (x *TableAzurePostgresqlServersGenerator) GetSubTables() []*schema.Table {
	return []*schema.Table{
		table_schema_generator.GenTableSchema(&TableAzurePostgresqlConfigurationsGenerator{}),
		table_schema_generator.GenTableSchema(&TableAzurePostgresqlFirewallRulesGenerator{}),
	}
}
