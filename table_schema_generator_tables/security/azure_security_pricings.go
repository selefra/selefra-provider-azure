...
func (x *TableAzureSecurityPricingsGenerator) GetDataSource() *schema.DataSource {
	return &schema.DataSource{
		Pull: func(ctx context.Context, clientMeta *schema.ClientMeta, client any, task *schema.DataSourcePullTask, resultChannel chan<- any) *schema.Diagnostics {
			svc := client.(*azure_client.Client).AzureServices().Security.Pricings
...
func (x *TableAzureSecurityPricingsGenerator) GetColumns() []*schema.Column {
	return []*schema.Column{
		...
		table_schema_generator.NewColumnBuilder().ColumnName("free_trial_remaining_time").ColumnType(schema.ColumnTypeString).Build(),
